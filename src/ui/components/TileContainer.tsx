      ...
      <div className="flex flex-col items-center w-full max-w-md px-6">
        ...
        {recentProjects.length > 0 && (
          <div className="mt-8 w-full">
            <p className="text-gray-500 text-xs mb-3">Recent projects</p>
            <div className="space-y-0.5 max-h-48 overflow-y-auto">
              {recentProjects.slice(0, 5).map((project) => (
                <button
                  key={project.id}
                  onClick={() => onSelectRecent(project.path)}
                  className="w-full flex items-center justify-between px-2 py-2 hover:bg-[#1e2a3a] rounded transition-colors text-left"
                >
